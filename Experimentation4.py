...
character1.stats["max health"] = 100
...
character2.stats["max health"] = 80
...
potion = Item(name = "Health Potion", quantity = 3, effect = heal_user(current_character, 20), description= "Heal self for 30 HP")
super_potion = Item(name = "Super Potion", quantity = 3, effect = heal_user(current_character, 30), description= "Heal self for 30 HP")
fire_potion = Item(name= "Fire Potion", quantity = 1, effect = harm_enemy(current_enemy, 20), description= "Deal 20 dmg to the enemy")
hearty_carrot = Item(name = "Hearty Carrot", quantity= 1, effect = increase_max_stat(current_character, 5), description= "Increase max health by 5")
...
Inventory = {
    ...
    hearty_carrot.name: hearty_carrot.quantity
}
...
def TurnResponse():
    if Action == "1":
        print (f"\nYou attacked the {current_enemy.stats["name"]}!")
        ...
        current_character.stats, current_item, current_item.quantity = ItemMenu()
...
def ItemMenu():
    ...
    Action = (input())
    if InventoryItems[int(Action)-1] == -1:
        ItemMenu()
    elif "Health Potion" in InventoryItems[int(Action)-1]:
        current_item = potion
        if current_character.stats["health"] == current_character.stats["max health"]:
            print ("You already have full health!")
            MainMenuDecision()
        elif (current_character.stats["health"] + 20) > current_character.stats["max health"]:
            potion.use_item()
            current_character.stats["health"] = current_character.stats["max health"]
    ...
    elif "Super Potion" in InventoryItems[int(Action)-1]:
        current_item = super_potion
        if current_character.stats["health"] == current_character.stats["max health"]:
            print ("You already have full health!")
        elif (current_character.stats["health"] + 30) > current_character.stats["max health"]:
            ...
            print ("You used a super potion and regenerated 30 health! You now have ", current_character.stats['health'])
        return (current_character.stats, current_item,current_item.quantity)
    elif "Fire Potion" in InventoryItems[int(Action)-1]:
        current_item = fire_potion
        print (f"You used a fire potion and dealt 20 damage!")
        ...
        if current_enemy.stats["health"] <= 0:
            ...
            quit()
        return (current_character.stats, current_item,current_item.quantity)
    elif "Hearty Carrot" in InventoryItems[int(Action)-1]:
        current_item = hearty_carrot
        print ("You used a heart Carrot!")
        current_item.use_item()
        return (current_character.stats, current_item,current_item.quantity)
    else:
        current_item = potion
        MainMenuDecision()
...
#Enemy Turn
    ...
    if character1.stats["health"] <= 0:
        ...
        print (f"{target_character.stats["name"]} health: {target_character.stats["health"]}/{ target_character.stats["max health"]}")
